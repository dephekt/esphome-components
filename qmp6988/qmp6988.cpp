...
// Physical-plausibility bounds. The QMP6988 spec range is 300..1100 hPa and -40..85 °C;
// these are widened slightly so genuine readings are never rejected, while the fixed
// garbage produced by a non-converting sensor (e.g. -3418 hPa / -61.7 °C) is caught.
static const float QMP6988_PRESSURE_MIN_HPA = 300.0f;
static const float QMP6988_PRESSURE_MAX_HPA = 1200.0f;
static const float QMP6988_TEMPERATURE_MIN_C = -50.0f;
static const float QMP6988_TEMPERATURE_MAX_C = 100.0f;
...
bool QMP6988Component::calculate_pressure_() {
  ...
  // All-zero data registers mean the device is not converting (e.g. it silently fell back to
  // sleep mode). Compensating these sentinels produces a large fixed garbage value, so flag it;
  // update() treats the resulting out-of-range reading as a trigger to re-initialize.
  if (p_read == 0 && t_read == 0) {
    ESP_LOGW(TAG, "Raw pressure/temp registers are zero — sensor is not converting");
  }
...
void QMP6988Component::update() {
  bool read_ok = this->calculate_pressure_();
  ...
  if (this->pressure_sensor_ != nullptr)
    this->pressure_sensor_->publish_state(pressure_hectopascals);
}
